...
class SecurityController extends ApiController implements ClassResourceInterface {
  ...
  public function registerAction(Request $request) {
    ...
    try {
      ...
      $entityPhone = $em->getRepository('AcmeDataBundle:Users')->findOneByPhone(StringUtility::formatPhoneNumber(trim($request->get('phone')), true));
      if($entityPhone)
        return ApiResponse::setResponse('A user with this phone number already exists.', Codes::HTTP_CONFLICT);
      ...
      $entity->setPhone(StringUtility::formatPhoneNumber(trim($request->get('phone')), true));
      ...
      if(!empty($customerData)) {
        ...
        //allocate card and update with customer info
        $cardNumber = Clutch::allocateCard($this->container->parameters['clutch']);
        if($cardNumber) {
          $userData = array(
          ...
          Clutch::allocateCustomerInfo($this->container->parameters['clutch'], $userData);
        }
      ...
      foreach($serviceReminders as $cars => $servs) {
        for($i=0;$i<count($servs);$i++) {
          if($servs[$i] == 'Meineke06')
            $serviceReminders[$cars][$i] = 'Oil Change Service Reminder';
          if($servs[$i] == 'Meineke07')
            $serviceReminders[$cars][$i] = 'Oil Change & Brake Service Reminder';
          if($servs[$i] == 'Meineke17')
            $serviceReminders[$cars][$i] = 'High Mileage Service Reminder';
          if($servs[$i] == 'Meineke19')
            $serviceReminders[$cars][$i] = 'Oil Change Reminder';
        }
      }

      $info['historyTransactions'] = $historyTransactions;
      $info['serviceReminders'] = $serviceReminders;
  ...
  public function registerCheckAction(Request $request) {
    ...
    try {
      ...
      unset($customerData['brandDemographics']);
