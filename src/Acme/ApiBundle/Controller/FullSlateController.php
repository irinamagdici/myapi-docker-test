...
class FullSlateController extends ApiController implements ClassResourceInterface {
  ...
  public function getStoreServicesFromFullSlateAction(Request $request, $id) {
    ...
    try {
      ...
      //call Full Slate API
      $result = FullSlate::getFullSlateServices($id,  $this->container->parameters['fullslate']['fullslate_url']);
  ...
  public function getStoreOpeningsFromFullSlateAction(Request $request, $id) {
    ...
    try {
      ...
      if(!strlen($timezone)) {
        ...
        $timezone = 'PDT';
        $res = FullSlate::getFullSlateOpenings($id, trim($request->get('services')), true, $this->container->parameters['fullslate']['fullslate_url']);
        if($res) {
      ...
      //call Full Slate API
      $result = FullSlate::getFullSlateOpenings($id, trim($request->get('services')), false, $this->container->parameters['fullslate']['fullslate_url']);
      ...
      for($i=0;$i<count($openings);$i++) {
        ...
        $entities['rawData'][$date->format('Y-m-d')][] = $date->format('h:i A');
        $entities['data'][$date->format('Y-m-d')] = $date->format('l, m/d/Y');
      }
  ...
  public function addStoreAppointmentAction(Request $request) {
    ...
    try {
      ...
      //call Full Slate API
      $result = FullSlate::saveFullSlateAppointment($request, $timezone, $this->container->parameters['fullslate']['fullslate_secured_url'], $this->container->parameters['fullslate']['fullslate_security_key'] );
